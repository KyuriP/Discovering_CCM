...
searchAM_KP <- function(amat,x,type = c("an","de","ant","sp","nb","pa","ch","pde", "pan"))
{
  ...
  ## check inputs
  if( type != "de" && type != "pde" && type != "an" && type != "ant" && type != "sp" && type != "nb" && type != "pa" && type != "ch" && type != "pan")
    stop("Unknown type")
  ...
  if( type == "sp" || type == "nb" || type == "pa" || type == "ch" ) {
    ...
    while (length(indD) > 0) {
      ...
      if( type == "de" ) {
        ...
        ## find all parents of d not visited yet
        #indR <- which(amat[d,] == 3 & amat[,d] == 2 & !isType) ## d <-- ... <-- r
        ## find all x s.t. d {<,o}--{-,o} r
        indR <- which((amat[d, ] == 3 | amat[d, ] == 1) &
                        (amat[, d] == 2 | amat[, d] == 1) & !isType)
      }
      indD <- c(indD, indR)
    }
    if( type == "ant" || type == "an" ) {  ## "ant" or "an" --> consider ---
      indD <- which(isType)
      while (length(indD) > 0) {
        ## next element in the queue
        d <- indD[1]
