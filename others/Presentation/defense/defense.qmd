---
title: "Discovering Cyclic Causal Models in Psychological Reesearch"
...
SHD stands for structural hamming distance, which is basically counting all the steps going from the estimated graph to the true graph. So lower SHD, the better, meaning that the estimated graph is closer to the true graph, hence taking less steps.
Here we see that overall the green line has a lower SHD value ...
...
But, we cannot say one specific algorithm outperforms in all different scenarios. And unfortunately our conclusion is rather nuanced. We believe that there is no single algorithm that is suitable for all scenarios. Researchers must consider the characteristics
